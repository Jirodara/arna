...
def autonomous_optimize(brain, state, config, perf, mode):
    ...
    # Demo modda sadece logla, uygulama
    if not is_real_mode(state):
        for a in actions:
            blog(f'📝 [DEMO ÖĞRENME] {a["desc"]} | {a["reason"]}', 'DEMO_LEARN')
            # Yine de kaydet — gerçeğe geçince referans olsun
    ...
    for a in actions:
        ...
        if ok:
            record_autonomous_decision(
                brain, a['type'], a['desc'], a['reason'],
                config_before, a['config'], 'real'
            )
            blog(f'✅ [OTONOM] Uygulandı: {a["desc"]}', 'AUTONOMOUS')
        else:
            blog(f'❌ [OTONOM] Uygulanamadı: {a["desc"]}', 'ERROR')
