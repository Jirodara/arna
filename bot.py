    ...
    try:
        ...
        try:
            scan_report = {
                ...
                'signals_found': len(final),
                'config': {
                    'tf': tf, 'min_score': min_score,
                    'trail_pct': config.get('trail_pct', 1.5),
                    'pos_usd': config.get('pos_usd', 400),
                    'pct_filter_on': pct_on,
                ...
                'signals': [{
                    ...
                    'score': s['score'],
                    'rsi': s.get('rsi', 0),
                    'trend_pass': s.get('trend_pass', False),
                    'rsi_ok': s.get('rsi_ok', False),
                    'vol_ok': s.get('vol_ok', False),
                    'boll_ok': s.get('boll_ok', False),
            ...
            _data = json.dumps(scan_report).encode()
            _req  = _ur.Request('http://localhost:8766/', data=_data,
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path in ('/brain', '/brain/state'):
            ...
            try:
                r=ur.urlopen('http://localhost:8766/brain',timeout=3); data=r.read()
            except: data=b'{}'
            self.send_response(200)
    ...
    def do_POST(self):
        ...
        if self.path in ('/brain', '/brain/state'):
            ...
            try:
                req=ur.Request('http://localhost:8766/brain',data=body,method='POST',headers={'Content-Type':'application/json'})
                r=ur.urlopen(req,timeout=3); data=r.read()
            except: data=b'{"ok":true}'
            self.send_response(200)
