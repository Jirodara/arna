...
def scan_market(state, config):
    ...
    try:
        ...
        if state.get('auto_trade') and final and not state.get('positions'):
            # Aktif saat kontrolü — sadece saat filtresi açıksa
            hours = state.get('active_hours', {'start': 0, 'end': 24, 'active': False})
            if hours.get('active', False):
                current_hour = datetime.utcnow().hour
                s, e = hours['start'], hours['end']
                # Gece yarısını geçen aralık desteği (örn. 23-04)
                if s < e:
                    in_range = s <= current_hour < e
                else:
                    in_range = current_hour >= s or current_hour < e
                if not in_range:
                    log_msg(state, f'⏰ OTO AL: Aktif saat dışında (UTC {current_hour}:xx) — giriş yapılmadı', 'gold')
                else:
                    best = final[0]
                        ...
                        def delayed_enter(sig, rem):
                            ...
                            with state_lock:
                                if state.get('auto_trade') and not state.get('positions'):
                                    ...
                                    if s2 < e2:
                                        in_r = s2 <= datetime.utcnow().hour < e2
                                    else:
                                        in_r = datetime.utcnow().hour >= s2 or datetime.utcnow().hour < e2
                                    if not h2.get('active') or in_r:
