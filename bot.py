...
def scan_market(state, config):
    ...
    try:
        ...
        if state.get('auto_trade') and final and not state.get('positions'):
            # Aktif saat kontrolü
            hours = state.get('active_hours', {'start': 9, 'end': 23})
            current_hour = datetime.now().hour
            if not (hours['start'] <= current_hour < hours['end']):
                log_msg(state, f'⏰ OTO AL: Aktif saat dışında ({current_hour}:xx) — giriş yapılmadı', 'gold')
            else:
                best = final[0]
                    ...
                    def delayed_enter(sig, rem):
                        ...
                        with state_lock:
                            if state.get('auto_trade') and not state.get('positions'):
                                h2 = state.get('active_hours', {'start': 9, 'end': 23})
                                if h2['start'] <= datetime.now().hour < h2['end']:
