...
# -*- coding: utf-8 -*-
"""
ARNA Bot — index.html ile birebir aynı mantık
...
def binance_signed(method, path, params, api_key, api_secret):
    ...
    try:
        if method == 'GET':
            ...
            r = requests.post(url, data=params, headers={'X-MBX-APIKEY': api_key}, timeout=10)
...
def vol_filt(klines):
    ...
    if avg3 < ma10 * 0.8:
        return {'pass': False, 'score': 0, 'ratio': r10, 'note': 'Son 3 mum MA10 %80 altı'}
...
def is_peak_recovery(klines, price, high24h):
    """index.html isPeakRecovery birebir"""
...
def has_fake_wick(klines, n=5):
    """index.html hasFakeWick birebir"""
    closed = klines[:-1][-n:]
    fake_count = 0
    for k in closed:
        body  = abs(k['c'] - k['o'])
...
# ── ÜST TF MAP (index.html ile birebir) ──
UPPER_TF_MAP = {
...
# ── ÖĞRENME SİSTEMİ (index.html ile birebir) ──
...
def apply_learning(score, tr, rsi, bol, vol, learn):
    """index.html applyLearning birebir"""
...
def clean_blacklist():
    now = time.time()
    to_del = [sym for sym, e in blacklist.items()
...
# ── MUM ZAMANLAMA (index.html waitForCandleClose birebir) ──
def check_candle_timing(symbol, tf):
    """Mum son %15'inde veya yeni açıldıysa True döner"""
    try:
...
def check_and_enter(state, config, signal, learn):
    """index.html checkAndEnter birebir — mum kapanınca 4 kontrol"""
    try:
...
# ══════════════════════════════════════════════════
# TARAMA — index.html startScan birebir
# ══════════════════════════════════════════════════
...
def scan_market(state, config):
    ...
    try:
        ...
        # ÖN FİLTRE (index.html ile birebir)
        cands = []
        for raw in tickers:
            sym   = raw['symbol']
            is_usdt = sym.endswith('USDT')
            is_try  = sym.endswith('TRY')
            if not (is_usdt or is_try):
                continue
            ...
            if is_try:
                high24h /= usd_try
            # TRY sembolleri USDT'ye çevir (index.html gibi)
        ...
        for c in cands:
            ...
            try:
                ...
                # ★ ÜST TF FİLTRESİ (index.html ile birebir)
                ...
                # ★ ZİRVE KONTROLÜ (index.html ile birebir)
                ...
                # ★ FİTİL TESPİTİ (index.html ile birebir)
                ...
                # RSI kontrolü (index.html ile birebir — <35 elensin)
                if rsi_r['rsi'] < 35:
                    add_blacklist(sym, False)
                    log_msg(state, f'🚫 {sym.replace("USDT","")[:6]} — RSI düşük ({rsi_r["rsi"]:.0f})', 'gold')
                    continue

                # Hacim kontrolü (index.html ile birebir)
                ...
                tp  = calc_tp(price, adj)

                # ★ MUM KAPANIŞI KONTROLÜ (index.html ile birebir — sadece 4h ve 1d)
                if tf in ('4h', '1d'):
        ...
        # Duplicate temizle (index.html ile birebir)
        best_by_base = {}
        for r in results:
            base = r['display_name']
        ...
        # Son 1 saatte SL ile kapanan coinleri filtrele (index.html ile birebir)
        now_ms = int(time.time() * 1000)
        ...
        # OTO AL/SAT — mum kapanışı bekle (index.html ile birebir)
...
# ══════════════════════════════════════════════════
# POZİSYON AÇMA/KAPAMA — index.html ile birebir
# ══════════════════════════════════════════════════
...
def update_positions(state, config):
    ...
    for pos in list(state['positions']):
        ...
        # Trailing SL güncelle (index.html ile birebir)
        ...
        if pnl_pct >= tp_pct * 0.5 and pos['trail_sl'] < pos['entry']:
            ...
            log_msg(state, f'🔒 BEP kilidi: SL girişe çekildi ({pos["display_name"]})', 'gold')
        ...
        # ANİ DÜŞÜŞ TESPİTİ (index.html ile birebir)
        if last_price > 0:
        ...
        # Momentum takibi (index.html ile birebir)
...
# ══════════════════════════════════════════════════
# POZİSYON ANALİZİ — index.html startPosCheck birebir
# ══════════════════════════════════════════════════
...
def check_position(state, config, pos):
    """index.html pozisyon analiz mantığı birebir"""
    sym = pos['symbol']
    tf  = pos.get('tf', '1h')
    lp  = pos.get('live_price', pos['entry'])
    pnl_pct = (lp - pos['entry']) / pos['entry'] * 100
...
# ── ÜST TF TAKİBİ (index.html ile birebir) ──
    ...
    def do_POST(self):
        ...
        with state_lock:
            ...
            if action == 'scan':
                ...
                # index.html'den gelen state güncellemesi
                incoming = cmd.get('state', {})
                for key in ['demo_bal', 'demo_pnl', 'positions', 'signals', 'history',
                            'mode', 'auto_scan', 'auto_trade', 'pos_exit_threshold']:
                    if key in incoming:
...
def main():
    ...
    pos_check_interval = 60    # 1 dakika (min TF için)
    ...
    # TF bazlı pozisyon analiz zamanlayıcıları
    pos_last_check = {}  # {pos_id: last_check_time}
    ...
    while True:
        try:
            ...
            # Pozisyon analizi — TF'ye göre farklı sıklık
            if now - last_pos_check >= 30:  # Her 30sn kontrol et, TF'ye göre filtrele
                with state_lock:
