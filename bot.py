...
def trend_filt(closes):
    ...
    # Korelasyon düzeltmesi: MA trend 20 puan (bağımsız), MACD 10 puan (tamamlayıcı)
    if up_trend and macd_up and macd_hist_pos:
        score = 25   # güçlü: her ikisi de onay
    elif up_trend and macd_up:
        score = 20   # trend + MACD line onayı
    elif up_trend:
        score = 12   # sadece MA trend
    else:
        score = 0
    return {
...
def calc_tp(price, score, sl_price=None):
    ...
    # Minimum 2:1 RR zorunluluğu
    if sl_price and sl_price > 0 and price > sl_price:
        sl_dist_pct = (price - sl_price) / price * 100
        min_tp_pct  = sl_dist_pct * 2.0   # 2:1 RR minimum
        high_tp_pct = sl_dist_pct * 2.5   # yüksek skor için 2.5:1
        if score >= 88:
            pct = max(pct, high_tp_pct)
        else:
            pct = max(pct, min_tp_pct)
        pct = min(pct, 8.0)  # max %8 cap
...
def record_trade(learn, result, tr_pass, rsi_ok, vol_ok, boll_ok, pnl_pct, tf):
    ...
    # R-multiple hesapla (profesyonel tracking)
    r_multiple = round(pnl_pct / abs(pnl_pct) if pnl_pct != 0 else 0, 2)  # basit versiyon
    entry = {
    ...
    try:
        ...
        for ci, c in enumerate(cands):
            ...
            try:
                ...
                # Regime filter — ADX ile sideways piyasayı ele
                try:
                ...
                if adj >= min_score:
                    # Son 30 kapanış fiyatı — grafik için
        ...
        if state.get('auto_trade') and final and not state.get('positions'):
            # Yasak saat kontrolü — sadece saat filtresi açıksa
            ...
            if hours.get('active', False):
                ...
                if in_pause:
                    log_msg(state, f'⏸️ OTO AL: Yasak saat (UTC {current_hour}:xx, {s}–{e} arası dur) — giriş yapılmadı', 'gold')
                else:
                    best = final[0]
                        ...
                        def delayed_enter(sig, rem):
                            ...
                            with state_lock:
                                if state.get('auto_trade') and not state.get('positions'):
                                    ...
                                    if s2 < e2:
                                        paused = s2 <= datetime.utcnow().hour < e2
                                    else:
                                        paused = datetime.utcnow().hour >= s2 or datetime.utcnow().hour < e2
                                    if not h2.get('active') or not paused:
                        ...
                        with state_lock:
                            if state.get('auto_trade') and not state.get('positions'):
                                ...
                                if s2 < e2:
                                    ...
                                    paused = datetime.utcnow().hour >= s2 or datetime.utcnow().hour < e2
                                if h2.get('active') and paused:
                                    log_msg(state, f'⏸️ OTO AL (delayed): Yasak saat — {sig["display_name"]} pas geçildi', 'gold')
                                    return
                                check_and_enter(state, config, sig, load_learn())
                    threading.Thread(target=delayed_enter2, args=(best, remaining_min), daemon=True).start()
...
def close_position(state, config, pos, exit_price, reason, learn=None):
    ...
    # Kill switch — 3 ardışık SL
    if reason == 'SL':
        state['consecutive_losses'] = state.get('consecutive_losses', 0) + 1
        if state['consecutive_losses'] >= 3:
            state['kill_switch'] = True
            state['auto_trade']  = False
            log_msg(state, '⛔ KILL SWITCH: 3 ardışık SL — OTO AL durduruldu! Manuel resetle.', 'dn')
    else:
...
def update_positions(state, config):
    ...
    for pos in list(state['positions']):
        ...
        # Trailing stop aktivasyon gecikmesi — TP'nin %40'ına kadar trailing dondur
...
def check_momentum_exit(state, config, pos):
    ...
    # TP'nin %90'ına ulaşılmadan devreye girme
    ...
    # Cooldown 90 saniye — daha az agresif
    if now - last_cd > 90:
        if momentum < -0.5:  # -0.3'ten -0.5'e çıkardık, daha az tetiklenir
            pos['_momentum_cd'] = now
            smart_tp = lp * 1.008  # biraz daha geniş TP
